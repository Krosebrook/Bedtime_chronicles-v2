...
export function useStoryAudio({ currentPart, storyMode, initialVoice, initialSpeed, modeVoices }: StoryAudioOptions): StoryAudioState {
  ...
  const speakCurrentPart = useCallback(async () => {
    ...
    try {
      ...
      if (__DEV__) console.log("Premium ElevenLabs TTS failed, falling back to on-device Expo Speech:", err);
      
      try {
        const rate = playbackSpeed === "gentle" ? 0.75 : playbackSpeed === "medium" ? 0.9 : 1.0;
        
        await Speech.stop();
        
        Speech.speak(currentPart.text, {
          rate,
          pitch: storyMode === "sleep" ? 0.85 : 1.0,
          onStart: () => {
            setIsSpeaking(true);
            setAudioLoading(false);
            setAudioPosition(0);
            setAudioDuration(currentPart.text.length * 70); // Rough approximation in ms
          },
          onDone: () => {
            setIsSpeaking(false);
            setAudioPosition(0);
          ...
          onError: (speechErr) => {
            if (__DEV__) console.log("Expo Speech error:", speechErr);
            setIsSpeaking(false);
            setAudioLoading(false);
          }
        });
      } catch (fallbackErr) {
        if (__DEV__) console.log("Expo Speech fallback error:", fallbackErr);
        setAudioLoading(false);
        setIsSpeaking(false);
      }
    }
